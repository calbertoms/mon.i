                ...
                <thead>
                    <tr>                        
                        ...
                        <th class="col-md-3" style="text-align: center; vertical-align: middle;">
                             <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#modalCad"><i class="fa fa-fw fa-plus"></i> Tanques Gasosos</button>
                        </th>
                    </tr>
                </thead>
                <tbody>
            ...
            <table class="table table-condensed">
                ...
                <tbody>
                    <?php foreach ($tanques as $t) {
                        ...
                        echo '<a style="margin-right: 1%" href="#modalEdit" class="btn btn-info editar" role="button" data-toggle="modal" tanqueGasoso="'.$t->idTanqueGasoso.'" title="Editar Tanque Gasoso"><i class="fa fa-fw fa-pencil"></i></a>';                                            
                        echo '<a style="margin-right: 1%"href="#modalExcluir" class="btn btn-danger excluir" role="button" data-toggle="modal" idTanqueGasoso="'.$t->idTanqueGasoso.'" title="Excluir Tanque Gasoso"><i class="fa fa-fw fa-remove"></i></a>';
                        echo '</td>';
                        echo '</tr>';
                    }?>
                </tbody>
        ...
        <div class="modal-content">
            ...
            <form id="formEdit" action="<?php echo base_url('TanqueGasoso_ctrl/editar'); ?>" method="post">
                <div class="modal-body">                    
                    <div class="row">
                        <div class="col-sm-12 col-md-12 col-lg-12">
                            <input type="hidden" name="idTanqueGasoso" id="idTanqueGasoso" value=""/>
                            <div class="row">
                                <div class="col-sm-12 col-md-12 col-lg-12">
                                    <div class="col-lg-12 alert alert-info">Obrigatorio o preenchimento dos campos com asterisco (<span style="color: #EE0000">*</span>).</div>
                                </div>
...
$(document).ready(function () {
    ...
    $(document).on('click', '.excluir', function () {
        var tanqueGasoso = $(this).attr('idTanqueGasoso');        
        $("#idExcluir").val(tanqueGasoso);        
    });
    
    //pega o id do funcionario que deseja editar e envia para o modal editar
    $(document).on('click', '.editar', function () {
        var tanqueGasoso = $(this).attr('tanqueGasoso');
        $("#idTanqueGasoso").val(tanqueGasoso); 
    });
    
    //carrega os dados do totem
    $('#modalEdit').on('shown.bs.modal', function () {
        var idTanqueGasoso = $('#idTanqueGasoso').val();
        $.ajax({
            type: 'POST',
            dataType: 'json',
            url: '<?php echo base_url('TanqueGasoso_ctrl/buscaTanque');?>',
            data: 'idTanqueGasoso='+idTanqueGasoso,
